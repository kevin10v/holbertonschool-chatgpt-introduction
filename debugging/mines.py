...
class Minesweeper:
    ...
    def reveal(self, x, y):
        # Jashtë kufijve ose tashmë e zbuluar → s'bëjmë asgjë
        ...
        # Nëse s’ka mina pranë, hap fqinjët (flood-fill)
    ...
    def all_safe_cells_revealed(self):
        """Kthen True nëse çdo qelizë pa minë është zbuluar."""
        for y in range(self.height):
            for x in range(self.width):
                idx = y * self.width + x
    ...
    def play(self):
        while True:
            ...
            try:
                ...
                # ✅ Kontrollo fitimin pas çdo lëvizjeje
